...
print(f"💾 2.4.14 dashboard_alerts.json → {dashboard_alerts_path_2414}")
print(f"   alerts: {n_alerts_2414} (critical: {n_critical_alerts_2414})")
```

># A:

You’re hitting the classic notebook problem: **you’re relying on a “global latest artifacts directory” (`SEC2_DIR`) but you never define it in the bootstrap.** So later sections explode with `NameError`.

If you want a **best long-term solution without using functions**, the answer is:

### ✅ Establish a single “Section 2 shared (latest) artifacts folder” as a *contract* in the bootstrap, and never deviate.
...
What you *don’t* have is a canonical **shared** “latest” folder. That’s what `SEC2_DIR` is trying to be.
...
print("📁 SEC2_DIR (shared latest):", SEC2_DIR)
```
...
print("📁 SEC2_24_DIR (2.4 owned):", SEC2_24_DIR)
```

Now your “two-tier artifact model” is stable:

* `SEC2_24_DIR` = outputs created by 2.4.x
* `SEC2_DIR` = published “latest” outputs for other sections

---

### 3) Replace your “publish” line with a plain inline block (no function)
...
does **not** belong where you put it **because `local_path` isn’t defined, and you only need to publish **after** the file is actually written successfully.
...
That’s enough. No function needed.
...
Because “latest inputs” are *artifacts* (JSON/CSV outputs from other sections), not the unified diagnostics log.
...
assert "SEC2_ARTIFACTS_DIR" in globals(), "❌ SEC2_ARTIFACTS_DIR missing. Run bootstrap Part 5."
assert "SEC2_ARTIFACT_DIRS" in globals(), "❌ SEC2_ARTIFACT_DIRS missing. Run bootstrap Part 6."
...
## Why this is the “best long-term” notebook approach

* **No functions needed**
* **No reliance on “cell order memory”** beyond running bootstrap once
* **One stable contract** for cross-section I/O: `SEC2_DIR/_latest`
* Avoids the “truth conflict” issue you already identified: *section-owned outputs vs shared published outputs*

---

If you want, I can point out the *exact* place in your bootstrap where `SEC2_DIR` should be defined so it’s guaranteed available for every subsequent section (it’s one line in Part 5 right after `SEC2_ARTIFACTS_DIR`).
